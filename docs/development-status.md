...
## 2026-09-24 Web のラベル・ステータス・親子関係

FUCHIKOMA-30〜33 に対し、設定画面からラベルとチームのステータスを追加・編集・削除できるようにしました。イシュー詳細ではラベルの付け外し、親の設定・変更・解除、直属の子の追加・解除・表示・移動ができます。使用中と最後のステータスは削除を拒否し、非公開チームの親子は権限のない利用者に返しません。実装は `50063b5` と `b4d7d0d` です。

`pnpm check` で実 Worker・CLI の96テスト、`pnpm test:e2e` でブラウザーの43テストが成功しました。最終レビューで見つけた親子候補の保存中競合を直し、遅延応答の追加 E2E と対象 API テストを再実行しました。`pnpm quality` と本番向けビルドも成功しています。

Cloudflare Worker のバージョン `09f39a96-a4b7-49fc-8ab2-a9c1c82d4900` に反映しました。本番では既存イシューを CLI で取得し、認証済みブラウザーでラベル設定とイシューの親子欄を確認しました。4件には結果をコメントし、すべて Done に更新済みです。判断と検証の記録は [作業記録](../.audit/web-classification-relations-plan.md) にあります。本番で大量の書き込みや性能測定は行っていません。
